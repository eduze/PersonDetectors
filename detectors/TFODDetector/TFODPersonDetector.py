...
class TFODPersonDetector:
    ...
    def __init__(self, model_path="./detectors/TFODDetector/models/faster_rcnn_inception_v2_coco_2017_11_08/frozen_inference_graph.pb", person_category=1,threshold=0.6, preview=False, useFaceDetection = False):
        '''
    ...
    def boxCheck(self, bounds, frame_shape):
        ...
        if width / frame_width > 0.4:
            return False
        if height / frame_height > 0.4:
            return False
        if width > height:
            return False
        return True
