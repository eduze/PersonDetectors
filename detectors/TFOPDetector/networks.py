...
def _get_base_path():
    if not os.environ.get('OPENPOSE_MODEL', ''):
        return './detectors/TFOPDetector/models'
...
def get_network(type, placeholder_input, sess_for_load=None):
    ...
    if sess_for_load is not None:
        if type == 'cmu':
            net.load('./detectors/TFOPDetector/models/numpy/openpose_coco.npy', sess_for_load)
        else:
            s = '%dx%d' % (placeholder_input.shape[2], placeholder_input.shape[1])
            ckpts = {
                # 'mobilenet': 'trained/mobilenet_%s/trained_model-release' % s,
                'mobilenet' : 'trained/trained_model-388003',
                'mobilenet_fast': 'trained/mobilenet_fast/trained_model-163000',
                'mobilenet_accurate': 'trained/mobilenet_accurate/trained_model-170000'
            }
